...
if grep -qF  "DOCNFS" /etc/exports ; then
  echo "Docker for Mac NFS already initialized. Skipping."
  exit 0
fi
...
sudo echo "# Docker for Mac NFS Entry - DOCNFS" | sudo tee -a $FILE > /dev/null
